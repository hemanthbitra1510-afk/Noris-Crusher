...
const VoucherSearch = ({ show, handleClose, handleSubmit }: SearchModalProps) => {
  ...
  const fetchParties = useCallback(async () => {
    try {
      ...
      const res = await axios.get(
        `https://norisapi.noris.in/Crusher/Masters.php?ID=${id}&Table=GetPartyLedgers`
      );
  ...
  return (
    <Modal show={show} onHide={handleClose} centered size="sm">
      <Modal.Header closeButton>
        <Modal.Title className="fw-bold">Voucher Search Filters</Modal.Title>
      </Modal.Header>
            ...
            <Col md={12}>
              <Form.Group>
                ...
                <Select
                  ...
                  isClearable
                  value={
                    partyOptions.find((opt) => opt.value === formData.Party) || null
                  }
                  onChange={(selected) =>
                    setFormData({
                      ...formData,
